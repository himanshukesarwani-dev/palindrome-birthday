...
var footerCredit = document.querySelector('footer')
...
function clickEventHandler() {
    ...
    footerCredit.style.marginTop = "30px"
    if (dateInput.value !== '') {
        ...
        if (checkPalidromeForAllFormats(date)) {
            ...
            imageStyle(celebrateImg);
            sadImg.style.display = "none";

        } else {
            [cP, pD] = getPreviousPalindromeDate(date);
            [cN, nD] = getNextPalindromeDate(date);
            resultBox.innerText = `Sorry!😢😢 It is not a Palindrome Date. 
            
            The last Palindrome Date was ${pD.day}-${pD.month}-${pD.year} and you missed it by ${cP} days.
            
            The next Palindrome Date is ${nD.day}-${nD.month}-${nD.year} and you missed it by ${cN} days`
            imageStyle(sadImg);
            celebrateImg.style.display = "none";

        }
    } else {
        resultBox.innerText = `Enter Your Date of Birth First! 😑`
    }
}
